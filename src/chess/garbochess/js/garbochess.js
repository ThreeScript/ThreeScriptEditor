      ...
      if (value > realEval) {
         if (value >= beta) {
            ...
            if (g_board[histTo] == 0) {
               ...
               historyTable[histPiece][histTo] += (ply ^ 2);
...
function InitializeFromFen(fen) {
   ...
   function chuncksMakeSquare(pieceStr, pieceLetter, chunkIndex, color, i, j1, j2, castleRights) {
      if (chunks[chunkIndex].indexOf(pieceLetter) !== -1) {
         if (g_board[MakeSquare(i, j1)] != (pieceKing | color) ||
                 g_board[MakeSquare(i, j2)] != (pieceRook | color)) {
            return 'Invalid FEN: ' + pieceStr + ' castling not allowed';
         }
         g_castleRights |= castleRights;
      }
   }
   chuncksMakeSquare("White kingside", "K", 2, colorWhite, 7, 4, 7, 1);
   chuncksMakeSquare("White queenside", "Q", 2, colorWhite, 7, 4, 0, 2);
   chuncksMakeSquare("Black kingside", "k", 2, colorBlack, 0, 4, 7, 4);
   chuncksMakeSquare("Black queenside", "q", 2, colorBlack, 0, 4, 0, 8);
   g_enPassentSquare = -1;
...
function MakeMove(move) {
   ...
   g_enPassentSquare = -1;
/*
   function testFlag(from1, from2, to1, to2) {
      if (IsSquareAttackable(from1, otherColor) ||
              IsSquareAttackable(from2, otherColor)) {
         g_moveCount--;
         return false;
      }

      var rook = g_board[to1];

      g_hashKeyLow ^= g_zobristLow[to1][rook & 0xF];
      g_hashKeyHigh ^= g_zobristHigh[to1][rook & 0xF];
      g_hashKeyLow ^= g_zobristLow[to2][rook & 0xF];
      g_hashKeyHigh ^= g_zobristHigh[to2][rook & 0xF];

      g_board[to2] = rook;
      g_board[to1] = pieceEmpty;

      g_baseEval -= pieceSquareAdj[rook & 0x7][me === 0 ? flipTable[to1] : (to1)];
      g_baseEval += pieceSquareAdj[rook & 0x7][me === 0 ? flipTable[to2] : (to2)];

      var rookIndex = g_pieceIndex[to1];
      g_pieceIndex[to2] = rookIndex;
      g_pieceList[((rook & 0xF) << 4) | rookIndex] = to2;
   }

   if (flags) {
      if (flags & moveflagCastleKing) {
         testFlag(from + 1, from + 2, to + 1, to - 1);
      } else if (flags & moveflagCastleQueen) {
         testFlag(from - 1, from - 2, to - 2, to + 1);
      }
   }
*/
   if (flags) {
...
function GenerateAllMoves(moveStack) {
   ...
   from = g_pieceList[pieceIdx++];
   while (from !== 0) {
      nextMove(from, 31, moveStack);
      nextMove(from, 33, moveStack);
      nextMove(from, 14, moveStack);
      nextMove(from, -14, moveStack);
      nextMove(from, -31, moveStack);
      nextMove(from, -33, moveStack);
      nextMove(from, 18, moveStack);
      nextMove(from, -18, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   ...
   from = g_pieceList[pieceIdx++];
   while (from !== 0) {
      nextMoveLoop(from, 15, moveStack);
      nextMoveLoop(from, -17, moveStack);
      nextMoveLoop(from, 15, moveStack);
      nextMoveLoop(from, 17, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   ...
   from = g_pieceList[pieceIdx++];
   while (from !== 0) {
      nextMoveLoop(from, -1, moveStack);
      nextMoveLoop(from, 1, moveStack);
      nextMoveLoop(from, 16, moveStack);
      nextMoveLoop(from, -16, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   // Queen quiet moves
   ...
   // King quiet moves
   {
      ...
      from = g_pieceList[pieceIdx];
      nextMove(from, -15, moveStack);
      nextMove(from, -17, moveStack);
      nextMove(from, 15, moveStack);
      nextMove(from, 17, moveStack);
      nextMove(from, -1, moveStack);
      nextMove(from, 1, moveStack);
      nextMove(from, -16, moveStack);
      nextMove(from, 16, moveStack);
      if (!g_inCheck) {
...
function nextMove(from, distance, moveStack) {
   var to = from + distance;
   if (g_board[to] === 0)
      moveStack[moveStack.length] = GenerateMove(from, to);
}

function nextMoveLoop(from, distance, moveStack) {
   var to = from - distance;
   while (g_board[to] === 0) {
      moveStack[moveStack.length] = GenerateMove(from, to);
      to -= distance;
...
function enemyMoveStack(from, distance, enemy, moveStack) {
   var to = from + distance;
   if (g_board[to] & enemy)
      moveStack[moveStack.length] = GenerateMove(from, to);
}

function enemyMoveStackLoop(from, distance, enemy, moveStack) {
   var to = from;
   do {
      to += distance;
   } while (g_board[to] === 0);
...
function enemyMovePawnTo(from, distance, enemy, moveStack) {
   var to = from + distance;
   if (g_board[to] & enemy) {
...
function pawnCaptures(inc, enemy, moveStack) {
   var pieceIdx = (g_toMove | 1) << 4;
   from = g_pieceList[pieceIdx++];
   while (from !== 0) {
      enemyMovePawnTo(from, inc - 1, enemy, moveStack);
      enemyMovePawnTo(from, inc + 1, enemy, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   if (g_enPassentSquare !== -1) {
...
function GenerateCaptureMoves(moveStack, moveScores) {
   ...
   pawnCaptures(inc, enemy, moveStack);

   // Knight captures
   pieceIdx = (g_toMove | 2) << 4;
   from = g_pieceList[pieceIdx++];
   while (from != 0) {
      enemyMoveStack(from, 31, enemy, moveStack);
      enemyMoveStack(from, 33, enemy, moveStack);
      enemyMoveStack(from, 14, enemy, moveStack);
      enemyMoveStack(from, -14, enemy, moveStack);
      enemyMoveStack(from, -31, enemy, moveStack);
      enemyMoveStack(from, -33, enemy, moveStack);
      enemyMoveStack(from, 18, enemy, moveStack);
      enemyMoveStack(from, -18, enemy, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   ...
   from = g_pieceList[pieceIdx++];
   while (from != 0) {
      enemyMoveStackLoop(from, -15, enemy, moveStack);
      enemyMoveStackLoop(from, -17, enemy, moveStack);
      enemyMoveStackLoop(from, 15, enemy, moveStack);
      enemyMoveStackLoop(from, 17, enemy, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   ...
   from = g_pieceList[pieceIdx++];
   while (from != 0) {
      enemyMoveStackLoop(from, -1, enemy, moveStack);
      enemyMoveStackLoop(from, 1, enemy, moveStack);
      enemyMoveStackLoop(from, -16, enemy, moveStack);
      enemyMoveStackLoop(from, 16, enemy, moveStack);
      from = g_pieceList[pieceIdx++];
   }
   ...
   // King captures
   {
      pieceIdx = (g_toMove | 6) << 4;
      from = g_pieceList[pieceIdx];
      enemyMoveStack(from, -15, enemy, moveStack);
      enemyMoveStack(from, -17, enemy, moveStack);
      enemyMoveStack(from, 15, enemy, moveStack);
      enemyMoveStack(from, 17, enemy, moveStack);
      enemyMoveStack(from, -1, enemy, moveStack);
      enemyMoveStack(from, 1, enemy, moveStack);
      enemyMoveStack(from, -16, enemy, moveStack);
      enemyMoveStack(from, 16, enemy, moveStack);
   }
}
